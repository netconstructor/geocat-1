<xsl:stylesheet version="1.0"
    ...
    <xsl:template mode="Citation" match="int:GM03_2Comprehensive.Comprehensive.CI_Citation|Core.Core.CI_Citation|int:citation">
        <gmd:CI_Citation>
            ...
            <xsl:for-each select="int:alternateTitle/int:GM03_2Core.Core.PT_FreeText">
                <gmd:alternateTitle>
                    <xsl:apply-templates mode="language" select="."/>
                </gmd:alternateTitle>
            </xsl:for-each>
            <xsl:if test="not(int:GM03_2Core.Core.CI_Date) and not(int:alternateTitle/int:GM03_2Core.Core.PT_FreeText)">
                <gmd:date/>
            </xsl:if>
            <xsl:for-each select="int:GM03_2Core.Core.CI_Date">
                <gmd:date>
            ...
            <xsl:for-each select="int:identifier|int:GM03_2Comprehensive.Comprehensive.CI_Citationidentifier/int:identifier">
                <gmd:identifier>
                    <xsl:apply-templates mode="Identifier"/>
                </gmd:identifier>
            </xsl:for-each>
            <xsl:for-each select="int:GM03_2Comprehensive.Comprehensive.CI_CitationcitedResponsibleParty">
            ...
            <xsl:for-each select="int:presentationForm">
                <gmd:presentationForm>
                    <gmd:CI_PresentationFormCode codeList="./resources/codeList.xml#CI_PresentationFormCode"
                                             codeListValue="{int:GM03_2Comprehensive.Comprehensive.CI_PresentationFormCode_/int:value}"/>
                </gmd:presentationForm>
            </xsl:for-each>
            <xsl:for-each select="int:series/int:GM03_2Comprehensive.Comprehensive.CI_Series">
                <gmd:series>
                    <gmd:CI_Series>
    ...
    <xsl:template mode="Citation" match="int:GM03_2Core.Core.CI_Date">
        <gmd:date>
            <xsl:apply-templates select="." mode="CitationDate"/>
        </gmd:date>
    </xsl:template>

    <xsl:template mode="CitationDate" match="int:GM03_2Core.Core.CI_Date">
        <gmd:CI_Date>
