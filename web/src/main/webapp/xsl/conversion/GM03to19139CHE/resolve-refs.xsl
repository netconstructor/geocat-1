...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="GM03Comprehensive.Comprehensive|GM03Core.Core" mode="ResolveRefs">
        <xsl:element name="{local-name(.)}" namespace="{namespace-uri(.)}">
            <xsl:choose>
                <xsl:when test="GM03Core.Core.MD_Metadata/parentIdentifier">
                    <xsl:apply-templates select="GM03Core.Core.MD_Metadata[parentIdentifier]" mode="ResolveRefs">
                        <xsl:with-param name="parent" select="name(.)"/>
                    </xsl:apply-templates>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates select="GM03Core.Core.MD_Metadata[1]" mode="ResolveRefs">
                        <xsl:with-param name="parent" select="name(.)"/>
                    </xsl:apply-templates>
                </xsl:otherwise>
            </xsl:choose>
            ...
            <xsl:apply-templates select="GM03Comprehensive.Comprehensive.formatDistributordistributorFormat"
    ...
    <xsl:key name="index" match="/TRANSFER/DATASECTION/GM03Comprehensive.Comprehensive//*|/TRANSFER/DATASECTION/GM03Core.Core//*" use="@TID"/>
    ...
    <xsl:template match="formatDistributor/@REF" mode="ResolveRefsCopy">
        <xsl:apply-templates mode="ResolveRefs" select="."/>
    </xsl:template>

    <xsl:template match="distributorFormat/@REF" mode="ResolveRefsCopy">
        <xsl:apply-templates mode="ResolveRefs" select="."/>
    </xsl:template>
