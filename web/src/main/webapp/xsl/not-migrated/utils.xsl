...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="socialBookmarks">
        ...
        -->
        <!-- Removed social networking tools #13168
    ...
    <xsl:template name="validIndicator">
        ...
        <xsl:choose>
                <xsl:when test="$indicator">
                    <img src="../../images/red-ball.gif" class="{$class}" alt="{/root/gui/strings/reusable/non-validated}" title=""/>
                </xsl:when>
                <xsl:otherwise>
                    <img src="../../images/green-ball.gif" class="{$class}" alt="{/root/gui/strings/reusable/validated}" title=""/>
                </xsl:otherwise>
            </xsl:choose>
    </xsl:template>
