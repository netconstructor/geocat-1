...
<!-- iso-internal-multilingual-conversion-url.xsl was copied from this file so fix bugs there too -->
...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="composeTranslations">
        ...
        <xsl:choose>
          ...
          <xsl:when test="normalize-space($elem/text())='' or not($elem)">
        ...
        <xsl:choose>
         <xsl:when test="count($elem/child::node()[normalize-space(text())!=''])>0">
          <gmd:PT_FreeText>
           <xsl:apply-templates mode="convert-inner-to-iso" select="$elem/*"/>
          </gmd:PT_FreeText>
         </xsl:when>
         <xsl:otherwise>
	...
	<xsl:template mode="convert-inner-to-iso" match="node()" priority="1">
    ...
    <xsl:template mode="convert-iso-to-inner" match="gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString">
    ...
    <xsl:template mode="convert-iso-to-inner" match="gco:CharacterString"><!--
        ...
        --><xsl:element name="{$metadataLang}"><xsl:value-of select="."/></xsl:element>
    </xsl:template>
