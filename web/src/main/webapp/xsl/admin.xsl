...
<!--							<tr>
					...
					<xsl:variable name="catalogueConfiguration">
						...
						<xsl:call-template name="addrow">
							<xsl:with-param name="service" select="'metadata.xsl.admin'"/>
							<xsl:with-param name="title" select="/root/gui/strings/xsl.admin"/>
							<xsl:with-param name="desc" select="/root/gui/strings/xsl.admin.des"/>
						</xsl:call-template>
