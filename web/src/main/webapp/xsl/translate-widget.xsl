...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="translationWidgetSelect" >
        ...
        <xsl:param name="class" />
          ...
          <xsl:variable name="DE" select="/root/gui/strings/de"/>
          <xsl:variable name="FR" select="/root/gui/strings/fr"/>
          <xsl:variable name="IT" select="/root/gui/strings/it"/>
          <xsl:variable name="EN" select="/root/gui/strings/en"/>
    ...
    <xsl:template name="translations" >
        ...
        <xsl:choose>
            <xsl:when test="$root/node()[substring(translate(name(),$LOWER, $UPPER),1,2)=$langCode]">
                <xsl:value-of select="$root/node()[substring(translate(name(),$LOWER, $UPPER),1,2)=$langCode]"/>
            </xsl:when>
            <xsl:when test="$root/node()[substring(translate(name(),$UPPER, $LOWER),1,2)=$langCode]">
                <xsl:value-of select="$root/node()[substring(translate(name(),$UPPER, $LOWER),1,2)=$langCode]"/>
            </xsl:when>
            <xsl:otherwise>
