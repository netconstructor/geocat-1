...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:gml="http://www.opengis.net/gml"
    ...
    <!-- Load labels. -->
    <xsl:variable name="label" select="document('../xml/schemas/iso19139/loc/eng/labels.xml')"/>
    <xsl:variable name="label.che" select="document('../xml/schemas/iso19139.che/loc/eng/labels.xml')"/>
    ...
    <xsl:template match="/">
        <html>
            <!-- Set some vars.-->
            <xsl:variable name="title" select="che:CHE_MD_Metadata/gmd:identificationInfo/che:CHE_MD_DataIdentification/gmd:citation/gmd:CI_Citation/gmd:title/gco:CharacterString"/>
            ...
            <body>
                ...
                <xsl:apply-templates mode="iso19139" select="che:CHE_MD_Metadata/*"/>
            </body>
        </html>
    </xsl:template>
    ...
    <xsl:template mode="iso19139" match="gmd:*[gco:CharacterString or gmd:PT_FreeText]|
        che:*[gco:CharacterString or gmd:PT_FreeText]|
    ...
    <xsl:template name="getTitle">
        <xsl:param name="name"/>
        <xsl:variable name="title" select="string($label.che/labels/element[@name=$name]/label)"/>
        <xsl:choose>
