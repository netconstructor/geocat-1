...
cgp.View = function(xmlLoader)
{
	...
	function getData()
	{
		...
		for (var i = 0; i < searchList.length; i++)
		{
			...
			searchData.push(
			{
				ANY_TEXT : xml.getElementById(divElem, 'cgp.anytext').value,
				FROM       : $F('cgp.from'),
				UNTIL      : $F('cgp.until'),
				LAT_NORTH : $F('cgp.latnorth'),
				LAT_SOUTH : $F('cgp.latsouth'),
				LON_EAST : $F('cgp.loneast'),
				LON_WEST : $F('cgp.lonwest')
			});
		}
	...
	function addSearch(search)
	{
		...
		Event.observe('cgp.from.clear', 'click', ker.wrap(this, function()
		{
			$('cgp.from') .value='';
		}));
		...
		Calendar.setup({
			inputField  : "cgp.until",
			ifFormat    : "%Y-%m-%d",
			button      : "cgp.until.set",
			showsTime   : false,
			align       : "Br",
			singleClick : true
		});

		Event.observe('cgp.until.clear', 'click', ker.wrap(this, function()
		{
			$('cgp.until').value='';
		}));
