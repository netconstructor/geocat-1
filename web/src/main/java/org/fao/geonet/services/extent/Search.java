...
public class Search extends List
{
    ...
    private DefaultQuery likeQuery(Element params, FeatureType featureType, String[] properties) throws IOException
    {
        ...
        StandardTokenizer tokenizer = new StandardTokenizer(new StringReader(pattern));
        StandardFilter standard = new StandardFilter(tokenizer);
        ISOLatin1AccentFilter latinFilter = new ISOLatin1AccentFilter(standard);
        LowerCaseFilter lowerCaseFilter = new LowerCaseFilter(latinFilter);

        Token next = lowerCaseFilter.next();
        while (next != null) {
            String string = String.valueOf(next.termBuffer(), 0, next.termLength());
            String token = updatePattern(method, string);
