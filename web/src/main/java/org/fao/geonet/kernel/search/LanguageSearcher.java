...
public class LanguageSearcher extends Searcher {
    ...
    public LanguageSearcher(String path, String locale, File luceneDir) throws CorruptIndexException, IOException {
        _searcher = new IndexSearcher(path);
        _stopWords = new HashSet<String>(Arrays.asList(new GeocatAnalyzer(locale, luceneDir).getStopWords()));
    ...
    public void search(Query query, Filter filter, HitCollector results) throws IOException {
        ...
        _searcher.search(modifiedQuery, filter, results);
    }
    ...
    public Hits search(Query query, Filter filter, Sort sort) throws IOException {
        ...
        return _searcher.search(modifiedQuery, filter, sort);
    }
    ...
    public Hits search(Query query, Filter filter) throws IOException {
        ...
        return _searcher.search(modifiedQuery, filter);
    }
    ...
    public void search(Query query, HitCollector results) throws IOException {
        ...
        _searcher.search(modifiedQuery, results);
    }
    ...
    public Hits search(Query query, Sort sort) throws IOException {
        ...
        return _searcher.search(modifiedQuery, sort);
    }
...
public void search(Weight weight, Filter filter, Collector results) throws IOException {
    ...
    public TopFieldDocs search(Weight weight, Filter filter, int n, Sort sort) throws IOException {
         if ((filter instanceof GeoNetworkCachingWrapperFilter) ) {
            ...
            if (cachedFilter instanceof DuplicateDocFilter) {
                DuplicateDocFilter df = (DuplicateDocFilter) cachedFilter;
               ...
               GeoNetworkChainedFilter cf = (GeoNetworkChainedFilter) cachedFilter;
                ...
                for (Filter f : cfList) {
                    if (f instanceof DuplicateDocFilter) {
    ...
    public Query removeStopWordTerms(Query original) {
        ...
        if (original instanceof TermQuery) {
            ...
            updated = newQuery;
        } else if (original instanceof RangeQuery) {
            RangeQuery query = (RangeQuery) original;
            Term lower;
            Query updatedTerm = updateTerm(original, query.getLowerTerm());
            if (updatedTerm == null || updatedTerm == original) {
                lower = query.getLowerTerm();
            } else {
                lower = null;
            }

            Term upper;
            updatedTerm = updateTerm(original, query.getUpperTerm());
            if (updatedTerm == null || updatedTerm == original) {
                upper = query.getUpperTerm();
            } else {
                upper = null;
            }

            updated = new RangeQuery(lower, upper, query.isInclusive());
