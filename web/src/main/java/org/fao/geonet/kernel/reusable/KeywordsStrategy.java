...
public final class KeywordsStrategy extends ReplacementStrategy
{
    ...
    public Map<String, String> markAsValidated(String[] ids, Dbms dbms, UserSession session) throws Exception
    {
        ...
        for (String id : ids) {
            ...
            concept = searcher.searchById(concept.getCode(), NON_VALID_THESAURUS_NAME, _currentLocale);

            for (String lang : concept.getLanguages()) {
                geocatThesaurus.addElement(concept.getCode(), concept.getLabel(lang), concept.getDefinition(), lang);
            }
