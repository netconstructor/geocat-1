...
public class ReusableObjManager
{
    ...
    public ReusableObjManager(String appPath, Iterator<Element> reusableConfigIter, SerialFactory serialFactory)
    {
        ...
        if (reusableConfigIter.hasNext()) {
            Element config = reusableConfigIter.next();
    ...
    {
        // the metadata ID
        String id = uuidToId(context, dm, uuid, idIsUUID);
        ...
        if (process != null) {
            ...
            if (changed != null) {
                XmlSerializer.update(dbms, id, changed);
            }
        }
        return process != null;
    }

    static String uuidToId(ServiceContext context, DataManager dm, String uuid, boolean idIsUUID)
    {
        String id = uuid;
