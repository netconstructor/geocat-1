...
public class MultiLingualIndexSupport
{
    ...
    {
    	this._styleSheetsDir = searchManager.getStylesheetsDir();
    ...
    public MultiLingualIndexSupport(File styleSheetsDir, File defaultLangDir)
    {
    	this._styleSheetsDir = styleSheetsDir;
        this._luceneBaseDir = defaultLangDir.getParentFile();
        this._defaultLangName = defaultLangDir.getName();
    }
            ...
            throws IOException
    {
        ...
        for (; i < searchers.length; i++) {
			searchers[i] = new LanguageSearcher(indices[i].getAbsolutePath(), locales[i], _styleSheetsDir);
        }
        if( i!=locales.length ){
            ...
            throws IOException
    {
        ...
        return new IndexWriter(index,
                new GeocatAnalyzer(locale, localeDir), true);
    }
    ...
    public IndexReader createMultiReader(String[] locales) throws IOException
    {
        ...
        for (; i < subReaders.length; i++) {
            subReaders[i] = IndexReader.open(indices[i].getAbsolutePath());
        }
        if( i!=locales.length ){
            throw new IllegalArgumentException("Not all locales requested are available"+Arrays.toString(locales));
        }
    ...
    public Document document(int doc, MapFieldSelector selector) {
